...
export class GraphService {
  ...
  async createAggregateNode(pipeline: Pipeline, component: Component) {
    ...
    await this.pipelineService
      ...
      .then((res: any) => {
        if(res.component.aggregateFunctions[0] !== undefined) {}
        {
          node.aggregateType =
            res.component.aggregateFunctions[0].aggregationType;
          node.column = res.component.aggregateFunctions[0].columnName;
          node.outputColumnName =
            res.component.aggregateFunctions[0].outputColumnName;
    ...
    await this.pipelineService
      ...
      .then((res: any) => {
       if(res.component!== undefined) 
        ...
        node.secondDataset = new Dataset(
          -1,
          res.component.secondTableName,
          "",
          ""
        );}
      });
